...
export async function POST(req: NextRequest) {
  try {
    ...
    // 生成问题的嵌入
    const questionEmbedding = await embeddings.embedQuery(currentMessageContent);
    ...
    // 转换为Document对象
    const langchainDocuments = (documents as any[]).map((doc) => {
      return new Document({
    ...
    // 模拟检索器，使用从PostgreSQL获取的文档
    const retrievalChain = async (question: string) => {
      resolveWithDocuments(langchainDocuments);
      return combineDocumentsFn(langchainDocuments);
    };
